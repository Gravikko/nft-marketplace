...
contract AuctionBiddingTest is AuctionTestBase {
    ...
    function test_MakeBid_RevertWhenOwner() public {
        ...
        vm.expectRevert(Auction.OwnerCannotBid.selector);
        vm.prank(USER1);
        auction.makeABid{value: DEFAULT_MIN_BID}(auctionId);
    }
    ...
    function test_SetAuctionFeeAmount() public {
        uint256 newFee = 200;
        vm.prank(multisig);
        auction.setAuctionFeeAmount(newFee);

        (,,,,, uint256 feeAmount,) = auction.getAuctionSettings();
        assertEq(feeAmount, newFee);
    }

    function test_SetAuctionFeeAmount_RevertWhenTooHigh() public {
        vm.expectRevert(Auction.InvalidAuctionFeeAmount.selector);
        vm.prank(multisig);
        auction.setAuctionFeeAmount(501);
    }
