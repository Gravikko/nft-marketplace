...
contract Factory_CollectionCreationTest is FactoryHelper {
    ...
    function test_RevertWhen_CreateCollectionWithoutVRFAdapter() public {
        // Use vm.store to set VRF adapter to zero (slot 3, after beacon at slot 2)
        bytes32 vrfAdapterSlot = bytes32(uint256(3));
        vm.store(address(factory), vrfAdapterSlot, bytes32(uint256(0)));
