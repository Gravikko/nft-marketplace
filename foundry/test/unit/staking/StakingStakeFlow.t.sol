...
contract StakingStakeFlowTest is StakingTestHelper {
    function test_StakeNFT() public {
        ...
        assertEq(staking.checkUserStakedNFT(USER1, collectionId, tokenId), true);
    }
    ...
    function test_UnstakeNFT() public {
        ...
        assertEq(staking.getUserStakedTokens(USER1, collectionId).length, 1);

        uint256 stakedTokenId = staking.getUserStakedTokens(USER1, collectionId)[0];
        ...
        assertEq(staking.checkUserStakedNFT(USER1, collectionId, tokenId), false);
        assertEq(staking.getStakedTimestamp(collectionId, tokenId), 0);
        assertEq(staking.getUserStakedTokens(USER1, collectionId).length, 0);
    ...
    function test_MultipleUsersStakingDifferentTokens() public {
        ...
        assertEq(staking.checkUserStakedNFT(USER1, collectionId, tokenId1), true);
        assertEq(staking.checkUserStakedNFT(USER2, collectionId, tokenId2), true);
        assertEq(staking.checkUserStakedNFT(USER1, collectionId, tokenId2), false);
        assertEq(staking.checkUserStakedNFT(USER2, collectionId, tokenId1), false);
    }
    ...
    function test_UnstakeAndRemoveFromArray() public {
        ...
        assertEq(staking.getUserStakedTokens(USER1, collectionId).length, 2);

        vm.warp(block.timestamp + 10 seconds);
        vm.prank(USER1);
        staking.unstakeNFT(collectionId, tokenId1);

        uint256[] memory remaining = staking.getUserStakedTokens(USER1, collectionId);
